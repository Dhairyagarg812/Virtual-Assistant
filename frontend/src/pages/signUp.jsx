        ...
        <div className="flex -z-30 items-center justify-center w-full h-screen  bg-cover" style={{ backgroundImage: `url(${bg})` }}>
            <div >
                <form onSubmit={handleClick} className="h-[70vh] w-[30vw] shadow-xl shadow-black bg-black/5 backdrop-blur-md flex flex-col items-center justify-center">
                    <h1 className="mb-[5vh] text-white font-bold text-3xl">Register to <span className="text-blue-500">Garg's Virtual Assistant</span></h1>
                    <input onChange={(e) => setName(e.target.value)} value={name} type="text" className=" mb-[3vh] text-lg w-[90%] rounded-4xl border-2 p-4 border-amber-50 text-white" placeholder="Enter Your Name"></input>
                    ...
                    <button className="flex items-center justify-center cursor-pointer mb-[1vh] w-10vw h-10vh bg-white p-3 text-lg font-bold  mt-2.5 w-[7vw] h-[6vh] rounded-full" type="submit">Sign Up</button>
                    <p onClick={() => navigate('/signin')} className="text-white text-lg ">Already have an account ?<span className="cursor-pointer ml-[1vh] text-blue-500">Sign In</span></p>
                </form>
            </div>
        </div>
    )
